...
public class NavegadorDeRegistro {
    ...
    public static String[] updateRegister(String db, String tbl, String id) throws Exception {
        Connection conexao = MySQLConnector.conectar();
        int idPerson = Integer.parseInt(id);
        int nextID = idPerson + 1;
        if (nextID >= 1) {
                String strSqlUpdateRegister = "select * from `" + db + "`.`" + tbl + "` order by `id` asc;";
                Statement stmUpdeteRegister = conexao.createStatement();
                ResultSet rstSqlUpdateRegister = stmUpdeteRegister.executeQuery(strSqlUpdateRegister);
                String[] result = {"", "", "", ""};

                while (rstSqlUpdateRegister.next()) {
                    if (id.equals(rstSqlUpdateRegister.getString("id"))) {
                        rstSqlUpdateRegister.next();
                        result[0] = rstSqlUpdateRegister.getString("id");
                        result[1] = rstSqlUpdateRegister.getString("nome");
                        result[2] = rstSqlUpdateRegister.getString("email");
                        result[3] = rstSqlUpdateRegister.getString("senha");
                        break;

                    }
                }
                rstSqlUpdateRegister.close();
                if (result[0] == "") {
                    return null;
                } else {
                    return result;
